...
namespace obelisk {
    ...
    auto multipart_meta_formdata = rule<class mat_parser, std::pair<std::string, std::string>>{"MultipartFormData"} = lit("form-data") > attr("form-data") > attr("form-data");
    ...
    bool http_body_parser(std::shared_ptr<http_request> &request) {
        if (boost::algorithm::icontains(request->content_type_, "multipart/form-data")) {
            ...
            request->raw_->read(data.data(), request->content_length_);
