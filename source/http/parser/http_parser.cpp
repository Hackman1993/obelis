...
namespace obelisk {
    ...
    auto multipart_meta_name = rule<class mat_parser, std::pair<std::string, std::string>>{"MultipartName"} =
            (no_case["name"] > attr(std::string("name")) > "=" > '"' > lexeme[*(char_ - '"')] > '"');
    auto multipart_meta_filename = rule<class mat_parser, std::pair<std::string, std::string>>{"MultipartFileName"} =
            (no_case["filename"] > attr(std::string("filename")) > "=" > '"' > lexeme[*(char_ - '"')] > '"');
    auto multipart_meta_formdata = rule<class mat_parser, std::pair<std::string, std::string>>{"MultipartFormData"} =
            lit("form-data") > attr("form-data") > attr("form-data");
    auto multipart_meta = rule<class mat_parser, std::pair<std::string, std::string>>{"MultipartMeta"} =
            multipart_meta_name|multipart_meta_filename|multipart_meta_formdata;

    auto http_urlencoded_parser = rule<class mat_parser, std::pair<std::string, std::string>>{"HttpUrlEncodedParser"} =
            *~char_("=&") >> -lit("=") >> *~char_("&");
    ...
    bool http_body_parser(std::shared_ptr<http_request> &request) {
        if(boost::algorithm::icontains(request->content_type_, "multipart/form-data")){
            ...
            std::string data;
            data.reserve(request->content_length_);
            request->raw_->seekg(std::ios::beg);
            auto bytes_read = request->raw_->readsome(data.data(), request->content_length_);
            if(bytes_read != request->content_length_){
                THROW(protocol_exception, "Content-Length not match stream size", "Obelisk");
            }
            return http_urlencoded_param_parser(request, data);
        }
        return true;
    }
    ...
    bool http_multipart_body_parser(std::shared_ptr<http_request> &request) {
        ...
        while(!request->raw_->eof()){
            ...
            if(!parse_result)
                THROW(protocol_exception, "Multipart form data meta parse failed!", "Obelisk");
            ...
            if (is_file) {
                ...
                if (!parse_result) {
                    THROW(protocol_exception, "Multipart form data Content-Type parse failed!", "Obelisk");
                }
            }
            std::getline(*request->raw_, line_data);
            if(line_data!="\r"){
            ...
            if(!is_file) {
                ...
                do{
                    auto bytes_read = request->raw_->readsome(data, 10240);
                ...
                request->raw_->seekg(-sbuf_.in_avail(), std::ios::cur);
                fstream->flush();
                fstream->close();
            }
    ...
    bool http_urlencoded_param_parser(std::shared_ptr<http_request> &request, std::string_view data) {
        ...
        bool parse_result = parse(data.begin(), data.end(),http_urlencoded_parser%'&', params);
        if(!parse_result)
